...
class Inferencer(BaseTrainer):
    ...
    def __init__(
        ...
        self.tacotron2._modules["decoder"].max_decoder_steps = 3000
    ...
    def process_batch(self, batch_idx, batch, metrics, part):
        ...
        sequences, lengths = self.utils.prepare_input_sequence(batch["text"], self.device == "cpu")

        with torch.no_grad():
            mel, _, _ = self.tacotron2.infer(sequences, lengths)
            audio = self.model(mel)

        audio = audio["prediction_audio"].cpu()
        rate = 22050

        if self.save_path is not None:
            torchaudio.save(self.save_path / "voice" / f"{batch['path'][0]}.wav", audio, rate)
