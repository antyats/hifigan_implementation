...
def cut_audio_len(audio, max_len):
    if max_len > audio.shape[1]:
        audio = pad(audio, (0, max_len - audio.shape[1]))
    else:
        random_start = np.random.randint(0, audio.shape[1] - max_len)
        audio = audio[:, random_start : random_start + max_len]
